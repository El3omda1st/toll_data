...
def extract_data_from_csv(csv_path):
    ...
    df.to_csv('/opt/airflow/dags/python_etl/staging/csv_data.csv',index=False)
    if os.path.exists('/opt/airflow/dags/python_etl/staging/csv_data.csv'):
...
def extract_data_from_tsv(tsv_path):
    ...
    df.to_csv('/opt/airflow/dags/python_etl/staging/tsv_data.csv',index=False)
def extract_data_from_fixed_width(fixed_width_path):
    ...
    extracted_data.to_csv('/opt/airflow/dags/python_etl/staging/fixed_width_data.csv', index=False)
def consolidate_data(csv_path, tsv_path, fixed_width_path):
    ...
    consolidated_data.to_csv('/opt/airflow/dags/python_etl/staging/extracted_data.csv', index=False)
...
def transform_data(data_path):
    ...
    extracted_data.to_csv('/opt/airflow/dags/python_etl/staging/transformed_data.csv', index=False)
...
with DAG(
    ...
    bash_command=(
        'curl -o /opt/airflow/dags/python_etl/staging/tolldata.tgz https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-DB0250EN-SkillsNetwork/labs/Final%20Assignment/tolldata.tgz '
        )
    )
    ...
    second_task = BashOperator(
        task_id='unzip_data',
        bash_command=(
            'tar -xzvf /opt/airflow/dags/python_etl/staging/tolldata.tgz -C /opt/airflow/dags/python_etl/staging'
        )
    )
    third_task = PythonOperator(
        task_id='extract_data_from_csv',
        python_callable=extract_data_from_csv,
        op_kwargs={'csv_path': '/opt/airflow/dags/python_etl/staging/vehicle-data.csv'},
    )
    fourth_task = PythonOperator(
        task_id='extract_data_from_tsv',
        python_callable=extract_data_from_tsv,
        op_kwargs={'tsv_path': '/opt/airflow/dags/python_etl/staging/tollplaza-data.tsv'},
    )
    fifth_task = PythonOperator(
        task_id='extract_data_from_fixed_width',
        python_callable=extract_data_from_fixed_width,
        op_kwargs={'fixed_width_path': '/opt/airflow/dags/python_etl/staging/payment-data.txt'},
    )
    sixth_task = PythonOperator(
        task_id='consolidate_data',
        python_callable=consolidate_data,
        op_kwargs={'csv_path': '/opt/airflow/dags/python_etl/staging/csv_data.csv',
                   'tsv_path': '/opt/airflow/dags/python_etl/staging/tsv_data.csv',
                   'fixed_width_path': '/opt/airflow/dags/python_etl/staging/fixed_width_data.csv'},
    )
    seventh_task = PythonOperator(
        task_id='transform_data',
        python_callable=transform_data,
        op_kwargs={'data_path': '/opt/airflow/dags/python_etl/staging/extracted_data.csv'}
    )
